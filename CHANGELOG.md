...
* **Fresh now works as a Git difftool** - `fresh $LOCAL $REMOTE` opens a LEFT/RIGHT comparison for both per-file and `--dir-diff` workflows; `fresh -d LEFT RIGHT` provides the same Vim-compatible entry point outside Git (#2105, reported by @asukaminato0721).
